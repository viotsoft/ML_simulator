...
Живой сайт: **https://mlsimulator.com** (плюс `www`, уводит на основной адрес).
Служебный адрес платформы — `https://ml-simulator-app-production.up.railway.app`;
он остаётся рабочим и постоянным редиректом уводит на боевой домен.

**Три переменные с адресами, и путать их нельзя:**

| Переменная | Что это | Когда менять |
| --- | --- | --- |
| `PUBLIC_URL` | канонический адрес сайта: canonical, `og:url`, `sitemap.xml`, редирект остальных хостов | при переезде на новый домен — сразу |
| `APP_URL` | служебный адрес: редиректы эквайринга, OAuth-колбэки соцсетей, публичные медиа `/m/` | только вместе с обновлением колбэков у площадок |
| `WFP_DOMAIN` | домен, зарегистрированный в кабинете WayForPay | только после смены домена в самом кабинете, иначе оплата перестанет проходить |

## Почта на домене: support@mlsimulator.com

Адрес указан в подвале обоих лендингов, в условиях использования, в политике
конфиденциальности и на сертификате. **Само приложение писем не отправляет** —
это чисто человеческий ящик для обращений, поэтому SMTP в коде не нужен.

На домене сейчас нет ни одной MX-записи, то есть письма не доходят. Домен куплен
через Railway и обслуживается в панели `name.com` — записи добавляются там.

Что выбрать, в порядке возрастания усилий:

1. **Пересылка (быстро, бесплатно).** В `name.com` включить email forwarding:
   `support@mlsimulator.com` → ваш личный ящик. Письма будут **приходить**, но
   отвечать вы будете со своего личного адреса — клиент увидит его, а не адрес
   поддержки.
2. **Zoho Mail, бесплатный тариф.** Полноценный ящик на своём домене: и приём,
   и отправка от имени `support@mlsimulator.com`. Нужно добавить MX, SPF и DKIM,
   которые Zoho выдаст при подключении домена.
3. **Google Workspace.** То же самое, но платно и с привычным интерфейсом Gmail.

Если нужна именно **отправка** от имени поддержки, вариант 1 не подходит —
пересылка работает только на приём. И в любом случае добавьте SPF и DKIM, иначе
письма от поддержки будут попадать в спам.
